...
class TencentCloudVectorStore(BaseVectorStore):
    """
    腾讯云向量数据库实现（预留接口）
    当配置中 tencent_cloud.enabled=true 时启用
    需要安装: pip install tcvectordb
    参考文档: https://cloud.tencent.com/document/product/1709
    """
    ...
    def __init__(self, config: Dict[str, Any], embedding_provider: EmbeddingProvider):
        ...
        self._client = None
        self._collection = None
        logger.info("TencentCloudVectorStore 初始化（预留模式）")
    ...
    def _ensure_client(self):
        ...
        if self._client is None:
            try:
                ...
                logger.info("腾讯云向量数据库客户端连接成功")
            except ImportError:
                logger.error("请安装 tcvectordb: pip install tcvectordb")
                raise
            except Exception as e:
    ...
    def create_collection(self, collection_name: str, **kwargs) -> Any:
        ...
        try:
            db = self._client.database(self.config.get("database", "insurance_db"))

            try:
                self._collection = db.collection(collection_name)
                logger.info(f"腾讯云向量集合已存在: {collection_name}")
            except Exception:
                ...
                index = Index(
                    ...
                    FilterIndex("content", FieldType.String, IndexType.FILTER),
                    FilterIndex("source", FieldType.String, IndexType.FILTER),
                    FilterIndex("page", FieldType.Uint64, IndexType.FILTER),
                    FilterIndex("chunk_index", FieldType.Uint64, IndexType.FILTER),
                    FilterIndex("chapter", FieldType.String, IndexType.FILTER),
                    FilterIndex("section", FieldType.String, IndexType.FILTER),
                    FilterIndex("vector", FieldType.Vector, IndexType.HNSW,
    ...
    def add_documents(self, documents: List[VectorDocument], batch_size: int = 100) -> bool:
        ...
        try:
            records = []
            for doc in documents:
                embedding = doc.embedding or self.embedding_provider.embed_single(doc.content)
                records.append({
                    "id": doc.id,
                    "content": doc.content,
            ...
            for i in range(0, len(records), batch_size):
                ...
                self._collection.upsert(documents=batch)

            logger.info(f"腾讯云向量写入完成: {len(documents)} 条")
            return True
        except Exception as e:
    ...
    def search(self, query_embedding: List[float], top_k: int = 10,
        ...
        try:
            ...
            results = self._collection.search(
                ...
                limit=top_k,
                filter=filter_expr,
                output_fields=["content", "source", "page", "chapter", "section", "chunk_index"]
            )
    ...
    def delete_by_filter(self, filter_metadata: Dict[str, Any]) -> int:
        ...
        try:
            ...
            affected = self._collection.delete(filter=filter_expr)
            logger.info(f"腾讯云向量删除: {affected} 条")
            return affected
        except Exception as e:
    ...
    def document_exists(self, doc_id: str) -> bool:
        """检查文档是否已存在"""
        try:
            results = self._collection.searchById(doc_id)
...
def create_vector_store(config: Dict[str, Any], embedding_provider: EmbeddingProvider) -> BaseVectorStore:
    ...
    if provider == "chroma":
        persist_dir = vs_config.get("persist_directory", "./chroma_db")
        os.makedirs(persist_dir, exist_ok=True)
        logger.info(f"创建 ChromaVectorStore: {persist_dir}")
        ...
        if not tc_config.get("enabled", False):
            logger.warning("腾讯云向量库未启用，回退到 ChromaDB")
            return ChromaVectorStore("./chroma_db", embedding_provider)
        logger.info(f"创建 TencentCloudVectorStore: {tc_config.get('host')}")
        return TencentCloudVectorStore(tc_config, embedding_provider)

    else:
        logger.warning(f"未知向量库 provider={provider}，回退到 ChromaDB")
        return ChromaVectorStore("./chroma_db", embedding_provider)
